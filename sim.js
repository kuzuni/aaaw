...
const TUNE={
  ...
  eBaseHp:22.458216, eBaseDmg:4.176345,
  ...
  eHpSeg:[[0,1.0292],[5,1.032343],[15,1.066673],[28,1.094817],[40,1.056146],[70,1.025948],[150,1.023303],[380,1.014496]],
  eDmgSeg:[[0,1.0265],[5,1.032343],[15,1.063007],[28,1.078089],[40,1.028607],[70,1.027867],[150,1.022773],[380,1.014496]],
  ...
  /* ⚑⚑⚑ T100 (주인 확정 2026-09-03) — `4+3*lv` → **`5*lv+1`**.
     ...
     고정 챕터의 공급 286(=75×3+9+2×26)이 이 285 를 딱 한 번 넘기게 잡혀 있어 **완주 = 특전 10개**이고,
     11레벨은 341 이 필요해 구조적으로 불가능하다. `expKill:3` 은 주인 지시대로 그대로다. */
  expNeed:lv=>5*lv+1,
};
...
/* ⚑ 주인 확정 제약 (PLAN §2.4, 2026-09-02 14:2X) — 전 500 챕터 공통 (⚑ T102 로 300 → 500):
   ...
   ⚑⚑⚑ T100 (주인 확정 2026-09-03) — 상한 **50 → 80**. 종전 «적 최대 50» 지시는 이 지시로 갱신됐다:
   새 경험치 곡선(5·Lv+1)에서 적 49 마리는 공급 205 < 필요 285 라 «완주 = 특전 10개» 가 산술적으로 불가능하고,
   주인이 «특전 10개 될 정도의 적 개수로 바꾸라» 고 명시했으므로 적 수를 76 으로 올렸다(상한은 76+여유 = 80).
   실제 구성은 아래 고정값으로 76 마리이고, 후반 난이도는 마릿수가 아니라 적 스탯으로 낸다는 조항은 그대로다. */
...
function chapterLayout(c){
  /* ⚑⚑⚑ T100 (주인 확정 2026-09-03) — 새 경험치 곡선(5·Lv+1) 위에서 «완주 = 특전 10개» 를 다시 맞춘 구성.
     **전 챕터 동일 고정 구성**이고 제비뽑기는 «순서» 에만 남는다:
     ...
     → 완주하면 정확히 특전 10개. 11레벨은 341 이 필요해 구조적으로 불가능하다.
     ⚑ 셈이 맞는 자리를 헷갈리지 말 것 — 보스 전까지의 공급은 75×3+2×26 = **277** 이라 레벨업은 **9번**이고,
       나머지 한 장은 **악마의 «다음 순번 앞당김»** 이 준다(보스 처치 레벨업은 주인 지시로 특전을 안 준다).
       그래서 쉼터·보스 경험치를 건드리면 10개가 곧바로 깨진다 — 게이트가 실측으로 감시한다.
     ⚑ 적 «수» 는 더 이상 난이도 노브가 아니다 — 난이도는 적 스탯(구간 성장률·벽 배수)으로만 만든다.
     ⚑ 덤: 챕터마다 적 수·쉼터 수가 흔들리던 것이 사라져 인접 챕터 난이도 역전(T28)이 구조적으로 없어진다.
     게이트 `tools/verifyChapterFixed.js` 가 챕터 1~500 전수로 이 구성과 «완주 = 특전 10개» 를 실측한다. */
...
/* ⚑ 주인 방향(2026-09-03) — 한 런에서 얻는 특전 수를 «풀 크기» 와 분리한다.
   ...
   ⚑ 챕터 레벨업 횟수(T100 = 10)와 반드시 같은 값이어야 한다 — 다르면 «완주 = 특전 N개» 가 깨진다.
...
const GT={
  ...
  /* ⚑⚑⚑ T102 (주인 확정 2026-09-03) — 등급별 1부위 기여 (0강·슬롯 0렙). 등급 배수 재설계본이다.
     ...
     검산(기본치 공25/체150/실250 + 6부위): 일반 50/250/400 · 희귀 100/500/800 · 영웅 200/1000/1600 ·
  ...
  atk:[4.167, 12.500,  29.167,  62.500,  395.833],
  hp: [16.667, 58.333, 141.667, 308.333, 1975.000],
  sh: [25.000, 91.667, 225.000, 491.667, 3158.333],
...
function grantNextPerk(G){
  ...
  const perk=PERKS[G.taken.length];              /* ⚑ 순서 = 배열 순서. 무작위·중복이 존재하지 않는다 (풀 ≥ 획득 수) */
  perk.ap(G.player);
  G.taken.push(perk);
  return perk;
}
...
/* ⚑⚑⚑ 실험1 = «난이도 사다리 8점» (주인 확정 2026-09-03 · T103 이 과녁 2점을 이 표로 갈아끼웠다) ----------
   **종전의 과녁 2점(표준 ch15 = 10% · 노장비 ch4 = 30%)은 주인 지시로 이 8점에 흡수됐다.**
   주인 원문 사다리: 노템 5 · 일반 15 · 희귀+슬롯5 28 · 영웅+슬롯10 40 · 전설+슬롯15 70 ·
   신화+슬롯25 150 · 신화9강+슬롯50 380 · 신화9강+슬롯100 **420**(⚑ 주인 정정 — 종전 600 폐기).
   8칸 전부 **클리어율 10% · 허용 오차 ±2%p** 이고, 판수는 과녁당 1,000판 이상 · 고정 시드 3벌이다.
   자(尺)에 «등급 고정» 이 없다 — 특전은 순서 획득이라 런마다 같은 순서로 10종이 붙는다.
   ⚑ 종전엔 실험1(과녁)과 실험5(사다리)가 서로 다른 표를 봤다 — 이제 **표가 하나**이고 실험5 는
     같은 8칸을 «슬롯 0렙» 진단으로 다시 보는 자리로만 남는다.
...
const EXP1_TARGETS=[
  ...
  {id:'영웅 풀셋·슬롯10',          rar:2, plus:0, slot:10,  at:40,  want:10},
  {id:'전설 풀셋·슬롯15',          rar:3, plus:0, slot:15,  at:70,  want:10},
  {id:'신화 풀셋·슬롯25',          rar:4, plus:0, slot:25,  at:150, want:10},
  {id:'신화+9강 풀셋·슬롯50',      rar:4, plus:9, slot:50,  at:380, want:10},
  {id:'신화+9강 풀셋·슬롯100',     rar:4, plus:9, slot:100, at:420, want:10},
];
/* 사다리 8칸의 확정 총 스탯 (주인 표 — 공/체/실. 진단 출력의 대조용이고 판정은 클리어율로만 한다) */
const LADDER_STAT={5:[25,150,250], 15:[50,250,400], 28:[108.9,543.4,868.9], 40:[239.3,1193.5,1908.5],
...
function exp2_retired(){
  ...
  console.log('  등급·선택창이 사라져 «등급 내 폭» 은 측정 대상이 없다. 새 과녁은 실험1(난이도 사다리 8점)이다.');
}
