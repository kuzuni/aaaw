    ...
    for(const n of G.nodes){
      if(!n.done&&(n.type==='rest'||n.type==='devil'||n.type==='angel')&&p.worldX>n.x-95){
        ...
        if(n.type==='rest'){
          ...
          gainExp(G,10);        /* SIM_REST_POLICY: 항상 경험치 (게이트 tools/verifyRestPolicy.js 가 감시) */
...
function exp1_rarityLadder(){
  const h=harness('EXP1_GEAR',0,2,0);
...
function exp2_perkWinrate(){
  const h=harness('EXP2_GEAR',0,9,0);
  /* T31 재보정 (2026-09-02 · 변별점 규칙 ②): 종전 «신화 6부위·슬롯 0렙» 은 스탯 사다리 개편 후 챕터8 클리어율이
     ...
     **T46 재보정 (시뮬 쉼터 = 항상 경험치 · 정본 ②④):** 쉼터 회복(최대체력 40%)이 시뮬에서 빠지자 같은 하니스가
     62.3% → 46.0% 로 목표 밴드(60~70%) 밖으로 나갔다. 정본 ② 가 «밴드 밖이면 하니스를 다시 골라라» 이므로
     1200판 재측정으로 다시 골랐다 — 일반+8 56.4% · **일반+9 69.3%** · 일반+10 82.8% → **일반+9 슬롯0 채택**.
