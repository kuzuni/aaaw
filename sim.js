...
const TUNE={
  ...
  /* ⚑ T35: 단일 성장률 `eHpG 1.185`·`eDmgG 1.08` 폐기 → PLAN §11.7 «구간별 성장률» 표.
     ...
     구간별 난이도 지수 a 로 재적합 — 성장률 = 1 + a*(T35 초기율 - 1), a = 1.0/1.15/1.6/1.45/2.0/2.7,
     기저는 40/8 → 26/5.2 (×0.65). 근거·실측표 `docs/balance/resume-R01/raw.md`. */
  eHpSeg:[[0,1.0718],[15,1.0544],[30,1.0565],[50,1.0724],[70,1.0330],[120,1.0149]],
  eDmgSeg:[[0,1.0497],[15,1.0544],[30,1.0349],[50,1.0345],[70,1.0338],[120,1.0138]],
...
function exp1_rarityLadder(){
  const h=harness('EXP1_GEAR',0,1,0), CH=hCh('EXP1_CH',11);
  ...
  /* T31 재보정 (2026-09-02, 스탯 사다리 개편 후 · 변별점 규칙 ③): 종전 «전설 6부위·슬롯 1렙» 은 개편으로 챕터6 클리어율이
     ...
     ⚑ **T1 R01 재보정 (난이도 재적합 · 정본 ②③④)**: 기저 40/8 → 26/5.2 로 챕터 6 이 통째로 물러져 일반+2 가 100.0% 천장 포화가 됐고,
       ...
       그래서 새로 연 «챕터» 축으로 재선정 — **챕터 11 · 일반+1 · 슬롯 0** (300판 실측: 혼합 42.7% ·
       사다리 일반 5.0 / 희귀 25.7 / 전설 73.0 / 신화 91.0%, 간격 20.7·47.3·18.0%p, 포화 0).
       후보 비교: 챕터 10·일반+0 은 신화 98.7% 로 천장(99%)에 붙고, 챕터 11·일반+0 은 일반 1.7% 로 바닥(1%)에 붙는다.
...
function exp2_perkWinrate(){
  const h=harness('EXP2_GEAR',0,2,0), CH=hCh('EXP2_CH',11);
  ...
  /* T31 재보정 (2026-09-02 · 변별점 규칙 ②): 종전 «신화 6부위·슬롯 0렙» 은 스탯 사다리 개편 후 챕터8 클리어율이
     ...
     **T49 재보정 (쉼터 보상 260/26 · 같은 세션):** 경험치가 +10 → +26 이 되자 일반+9 가 80.7%(게이트 300판)로
     ...
     ⚑ **T1 R01 재보정 (난이도 재적합 · 정본 ②④)**: 기저 40/8 → 26/5.2 로 챕터 8 이 물러져 일반+8 이 100.0% 천장 포화가 됐다.
       실험1 과 같은 이유로 «챕터» 축을 써서 재선정 — **챕터 11 · 일반+2 · 슬롯 0**.
       실측: 게이트 300판 64.3% · 채점이 실제로 도는 1200판 시드1 64.6% / 시드2 63.1% 로 목표 밴드 60~70% 정중앙이다
       (같은 챕터 11 에서 일반+1 은 42.7% 로 낮다). 종전 등재: 챕터 8 · 일반+8 · 65.0%.
