...
/* ⚑⚑⚑ T97 — 실험1 출력이 «등급 과녁 3칸» 에서 **난이도 과녁 2점**으로 바뀌었다(주인 확정 2026-09-03).
   옛 파서는 «일반|희귀|전설» 행을 찾았고 T96 이후로는 그 행이 없어 표가 빈 채로 찍혔다.
   새 판정표 행: `| 표준 장비(희귀 풀셋·슬롯0) | 15 | 10% | 8.3% | -1.7%p | ✓ |` */
function parse1(out) {
  const m = {};
  for (const g of out.matchAll(/^\s*\|\s*(표준 장비[^|]*?|노장비[^|]*?)\s*\|\s*(\d+)\s*\|\s*(\d+)%\s*\|\s*([\d.]+)%\s*\|/gm))
...
/* ⚑⚑⚑ T97 — 판정이 붙는 것은 **실험1 의 과녁 2점뿐**이다(허용 ±2%p · sim.js EXP1_TOL 과 같은 값).
   실험2(등급 내 폭)는 폐지됐고, 실험5(사다리 7점)는 주인 지시로 **기준이 폐기**돼 진단 표시만 남긴다
   — 숫자는 계속 찍되 ✓/✗ 를 달지 않는다(달면 폐기된 기준으로 판정하는 것이 된다). */
const JUDGE = {
  '1': (k, v, t) => (t === null ? '' : (Math.abs(v - t) <= 2 ? '✓' : '✗')),          // 과녁 ±2%p (주인 확정)
  '2': () => '',                                                                      // 폐지 (T96)
  '5': () => '',                                                                      // 기준 폐기 — 진단 표시만 (T97)
};
const TITLE = { '1': '실험1 난이도 과녁 2점', '2': '실험2 (폐지)', '5': '실험5 스탯 사다리 (진단 — 기준 폐기)' };
