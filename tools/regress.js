...
/* ⚑⚑⚑ T103 — 실험1 출력이 «과녁 2점» 에서 **난이도 사다리 8점**으로 바뀌었다(주인 확정 2026-09-03).
   ⚠ 칸 이름을 정규식에 열거하지 않는다 — T97 파서는 «표준 장비|노장비» 를 박아 뒀다가 칸 이름이
   바뀌자 표가 통째로 비었다(그 전엔 «일반|희귀|전설» 로 같은 사고를 냈다). 이제 **표의 모양**으로 잡는다:
   실험1 판정표 행은 6칸이고 5번째 칸이 «편차 %p» 라 그것을 닻으로 쓴다.
   행 예: `| 희귀 풀셋·슬롯5 | 28 | 10% | 10.4% | +0.4%p | ✓ |` */
function parse1(out) {
  const m = {};
  for (const g of out.matchAll(/^\s*\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|\s*(\d+)%\s*\|\s*([\d.]+)%\s*\|\s*[+-][\d.]+%p\s*\|/gm))
    m[`${g[1]} ch${g[2]}`] = { target: num(g[3]), v: num(g[4]) };
  return m;
}
/* 실험5 요약행은 3칸(`| 칸 | 챕터 | 클리어율 |`)이라 «줄 끝» 으로 실험1 표와 구분된다. */
function parse5(out) {
  const m = {};
  for (const g of out.matchAll(/^\s*\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|\s*([\d.]+)%\s*\|\s*$/gm))
...
/* ⚑⚑⚑ T97 — 판정이 붙는 것은 **실험1 의 과녁 2점뿐**이다(허용 ±2%p · sim.js EXP1_TOL 과 같은 값).
   실험2(등급 내 폭)는 폐지됐고, 실험5(같은 8칸의 슬롯 0렙 관측)는 진단이라 표시만 남긴다
   — 숫자는 계속 찍되 ✓/✗ 를 달지 않는다(달면 판정 자가 둘이 된다). */
const JUDGE = {
  '1': (k, v, t) => (t === null ? '' : (Math.abs(v - t) <= 2 ? '✓' : '✗')),          // 사다리 8점 ±2%p (주인 확정)
  '2': () => '',                                                                      // 폐지 (T96)
  '5': () => '',                                                                      // 진단 표시만 (판정은 실험1)
};
