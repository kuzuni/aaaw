...
const CASES = [
  ...
  ['① §4 소환 계수 산문 — 도끼 0.30→0.35', 'PLAN',
    '도끼 한 개당 공격력의 **0.30**', '도끼 한 개당 공격력의 **0.35**', 1],
  ['② §4 소환 계수 산문 — 화살 0.50→0.65', 'PLAN',
    '화살 한 발당 **0.50**', '화살 한 발당 **0.65**', 1],
  ['③ §4 소환 계수 산문 — 번개 0.75→0.90', 'PLAN',
    '번개 한 회당 **0.75**', '번개 한 회당 **0.90**', 1],
  /* ⚠ 창(R_SPEAR 1.00)으로는 이 시험이 성립하지 않는다 — fireSpear 본문의 `n=n||1` 이 풀에 1 을 넣어
     산문의 «1.00» 이 늘 설명돼 버린다. 그래서 검기(0.50)로 엔진쪽 드리프트를 시험한다. */
  ['④ 엔진쪽 드리프트 — R_WAVE 0.50→0.55 (산문은 그대로 0.50)', 'SIM',
    'R_WAVE=0.50', 'R_WAVE=0.55', 1],
