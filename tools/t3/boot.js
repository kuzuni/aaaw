...
(async () => {
  ...
  for (const vp of [{ width: 390, height: 844 }, { width: 360, height: 800 }, { width: 375, height: 667 }]) {
    ...
    chk('⚑ T155 특전 100종 로드 (일반 39 · 희귀 32 · 전설 29)', boot.perks === 100 && boot.perkGrades === '39/32/29',
    ...
    /* ⚑ T116 U02 — 리터럴만 24 → 20 (레퍼런스 제목 폭 31.3% 에 맞추며 줄인 값).
       지키는 성질은 그대로다: «프레임 폭에 연동하지 않는 고정 px» — vw/clamp 로 바뀌면 이 단언이 깨진다. */
    chk('로비 제목 글씨가 축소되지 않는다 (CSS 20px 그대로)', mob.titleFs === '20px', mob.titleFs);
    ...
    /* --- ⚑ 대형 수치 표기 (T54) — 실제 렌더로 «후반 챕터 골드가 줄을 밀어내지 않는가» ---
       ...
       구 구현(콤마 전체 표기)은 후반 챕터 골드가 수십 자라 줄이 부풀어 🔊 가 화면 밖으로 나갔다. ⚑ T103 으로 상한이 420 이 됐으니 최대 챕터로 본다. */
    for (const c of [40, 90, 420]) {
  ...
  console.log('\n=== ⚑ T64 좁은 프레임 상단 줄 (프레임 278 · 267 · 269px) ===');
  {
    ...
    for (const vp of [{ width: 375, height: 587 }, { width: 360, height: 563 }, { width: 320, height: 568 }]) {
      ...
      await p.evaluate(() => { save.maxChapter = 420; save.selChapter = 420; startChapter(420); });   /* ⚑ T103 */
