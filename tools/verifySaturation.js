...
/* «효과 포화» 자동 대조 게이트 (T19 신설)
   ...
   방식:
     ...
     ② GOPT 126칸(⚑ T124 → ⚑ T153 — 3세트 × 6부위 × 7옵션)에서 각 옵션이 ++ 하는 px 키와 그 옵션의 인덱스(→ 요구 등급/강화)를 뽑는다.
...
/* ⚑⚑⚑ T153 — 영웅 폐지 + 맨 끝 칸 삭제로 7단이다. */
const reqOf = i => ['일반', '희귀', '전설', '신화 0강', '신화 +3강', '신화 +6강', '신화 +9강'][i] || `옵${i + 1}`;
...
if (nSeries !== 18 || nOpt !== 126) {
  console.error(`✗ 종류/칸 수가 18종류 126칸이 아니다 (${nSeries}/${nOpt}) — 파싱이 깨졌거나 스펙이 바뀌었다`);
  process.exit(1);
}
