...
const BASELINE = {
  ...
  /* ⚑⚑⚑ 재보정 = **실험2 하니스 재선정** (P3 R01 · T85, 2026-09-03 / 워커 C) — 사유: 주인 확정 P3 ①단계
     «일반 영점». 적 기저를 26.82/4.986 → **40.0/7.44**(×1.49) 로 올려 실험1 «일반 고정» 을 78.3 → 8.4%
     (과녁 10±5)로 맞춘 회차다. 난이도 노브를 움직였으므로 정본 ④ 가 재보정을 강제한다.
     · **실험1 하니스는 유지** — 26.0% 로 변별 구간 15~85% 안이고 사다리 3단이 7.0 / 24.3 / 70.7%
       (포화 0, 인접 간격 17.3·46.4%p). 애초에 이 하니스가 P3 ①의 영점 대상이라 옮길 수 없다.
     · **실험2 하니스는 90.3 → 36.7% 로 밴드 아래로 빠져 다시 골랐다** — 표준 장비가 주인 확정 «희귀 풀셋»
       이라 조절 축은 슬롯뿐이다. 이 게이트 300판×12시드 실측: 슬롯 5 = 36.7 · 14 = 55.0 · **15 = 63.0** ·
       16 = 62.0 · 18 = 68.7 → 밴드(60~70%) 정중앙이자 균등보너스 계단인 **슬롯 15 채택**.
     종전 등재: e1 = 51.3/-48.7 · e2(슬롯 5) = 66.7/-33.3. 실측표는 PROGRESS T85 행 · `docs/balance/T85/raw.md`. */
  e1: { ch: 30, har: { rar: 1, plus: 0, slot: 0 }, rate: 26.0, gap: -55.8 },
  e2: { ch: 30, har: { rar: 1, plus: 0, slot: 15 }, rate: 63.0, gap: -18.8 },
};
