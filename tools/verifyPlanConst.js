...
const CHECKS=[
  ...
  /* ⚑⚑⚑ T103 — 최종 벽 위치가 600 → 420 (주인 정정). 임계 자체도 엔진 상수(`TUNE.wall4At`)라 대조 대상이다:
     문면과 임계가 따로 놀면 «PLAN 은 420 인데 엔진은 600 에서 벽» 을 아무도 못 잡는다.
     ⚠ 배수는 T103 재적합에서 ×1.00(꺼짐)이 됐다 — 아래 wall4Hp/Dmg 대조가 그 값을 PLAN §6 과 맞춰 본다. */
  ['wall4At',  T('wall4At'),  /(\d+) 최종 벽 : c>=\d+ → HP ×[\d.]+/,                        '§6 확정값',1,CONF],
  ['wall4At(임계)',T('wall4At'),/\d+ 최종 벽 : c>=(\d+) → HP ×[\d.]+/,                      '§6 확정값',1,CONF],
  ['wall4Hp',  T('wall4Hp'),  /\d+ 최종 벽 : c>=\d+ → HP ×([\d.]+)/,                        '§6 확정값',1,CONF],
  ['wall4Dmg', T('wall4Dmg'), /\d+ 최종 벽 : c>=\d+ → HP ×[\d.]+ · DMG ×([\d.]+)/,          '§6 확정값',1,CONF],
...
const TABLE=[];
{
  ...
  /* ⚑⚑⚑ T103 (2026-09-03) — 구간 경계를 **사다리 8점의 과녁 챕터에 맞춰 새로 잘랐다**: 5·15·28·40·70·150·380·420.
     종전 경계(15/20/30/50/70/120/260)는 옛 사다리 7점 것이라 새 과녁과 어긋나 있었고, 그 상태로는
     «한 과녁을 맞추면 옆 과녁이 밀리는» 구조가 남는다. 경계를 과녁에 맞추면 구간 하나가 과녁 하나만
     담당해 앞에서부터 순서대로 자를 수 있다. 9줄 → **8줄**이 됐으니, 이 배열과 PLAN §11.7 표의
     행 순서가 어긋나면 색인이 통째로 밀린다(그래서 여기서 이름으로 대조한다). */
  /* ⚑⚑⚑ T160 (주인 확정 2026-09-05 20:2X «맞춰라») — 새 사다리 7점에 맞춰 경계를 다시 잘랐다:
     3·7·15·30·60·100. 마지막 구간 [100, …] 은 과녁 125 를 지나 **420 까지 그대로 이어진다**
     (주인 20:3X «자르지는 말고» — 최종 벽은 꺼진 채 위치만 420). 8줄 → **7줄**. */
  const SEG=[['1','3'],['3','7'],['7','15'],['15','30'],['30','60'],['60','100'],['100','420']];
  SEG.forEach(([a,b],i)=>{
