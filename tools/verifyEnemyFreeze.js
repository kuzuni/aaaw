...
/* ================================================================
   ...
   ================================================================ */
const FROZEN = {
  eBaseHp: 50.174688, eBaseDmg: 9.330519,      /* 기저 (§6 확정값) */
...
/* 동작 동결 — enemyStats(c,w) 실측 [챕터, 웨이브, HP, DMG].
   표본 챕터는 구간 경계(5·15·28·40·70·150·380)와 벽(10·15·90·420) **직전·직후**를 다 덮는다:
   상수를 그대로 두고 임계만 옮겨도(예: c>=10 → c>=12) 여기서 걸리게 하려는 것. */
const FROZEN_STATS = [
  [1,0,50,9],[1,3,73,12],[4,0,55,10],[4,3,79,13],[5,0,56,10],[5,3,82,13],
  [9,0,57,10],[9,3,83,13],[10,0,86,13],[10,3,124,16],[14,0,87,13],[14,3,126,17],
  [15,0,87,13],[15,3,126,17],[27,0,200,29],[27,3,290,37],[28,0,215,31],[28,3,311,39],
  [39,0,487,60],[39,3,707,75],[40,0,525,64],[40,3,762,79],[69,0,2351,133],[69,3,3409,165],
  [70,0,2476,137],[70,3,3590,169],[89,0,3834,219],[89,3,5560,272],[90,0,7847,337],[90,3,11379,418],
  [149,0,30526,1464],[149,3,44263,1816],[150,0,31237,1501],[150,3,45294,1862],
  [379,0,5795499,247363],[379,3,8403473,306730],[380,0,5929206,252939],[380,3,8597349,313644],
  [419,0,10455562,446032],[419,3,15160565,553080],[420,0,10608747,452567],[420,3,15382683,561183],
  [500,0,33962457,1448831],[500,3,49245563,1796551],
];
...
if (process.argv.includes('--self')) {
  ...
  const cases = [
    ['기저 HP·DMG 를 재적합하듯 두 엔진 다 바꾸면 (PLAN 까지 같이 고쳐도 여기선 걸린다)',
      s => s.replace('eBaseHp:50.174688, eBaseDmg:9.330519', 'eBaseHp:44.000000, eBaseDmg:8.100000'),
      s => s.replace('eBaseHp:50.174688, eBaseDmg:9.330519', 'eBaseHp:44.000000, eBaseDmg:8.100000'), null],
    ['10챕터 벽 배수를 올리면',
    ...
    ['구간 성장률 한 칸만 «미세 조정» 하면 (28→40 HP 7.7458% → 7.0%)',
      s => s.replace('[28,1.077458]', '[28,1.070000]'),
      s => s.replace('[28,1.077458]', '[28,1.070000]'), null],
    ...
    ['index.html 만 흔들면 (두 엔진이 갈라진다)',
      null, s => s.replace('eBaseHp:50.174688', 'eBaseHp:52.000000'), null],
