...
/* ================================================================
   ...
   ================================================================ */
const FROZEN = {
  /* ⚑⚑⚑ T160 재적합 (주인 확정 2026-09-05 20:2X «맞춰라» — 상시 규칙의 명시 예외).
     새 사다리 7점(3·7·15·30·60·100·125)에 맞춰 기저와 구간 경계를 다시 잘랐다.
     벽은 한 글자도 안 움직였다(10 벽 1.5/1.25 · 15 벽 꺼짐 · 90 벽 2.0/1.5 · 최종 벽 꺼짐 · 위치 420).
     종전 동결값(T103): 기저 50.174688 / 9.330519 · 경계 5·15·28·40·70·150·380. */
  eBaseHp: 49.283586, eBaseDmg: 9.164809,      /* 기저 (§6 확정값 · ⚑ T160) */
...
const FROZEN_STATS = [
  [1,0,49,9],[1,3,71,11],[2,0,51,9],[2,3,74,12],[3,0,52,10],[3,3,76,12],
  [6,0,78,14],[6,3,113,18],[7,0,89,16],[7,3,129,20],[9,0,96,18],[9,3,139,22],
  [10,0,149,23],[10,3,216,28],[14,0,171,26],[14,3,249,33],[15,0,178,27],[15,3,258,34],
  [29,0,742,106],[29,3,1075,131],[30,0,821,117],[30,3,1191,145],[59,0,7346,540],[59,3,10652,670],
  [60,0,7923,569],[60,3,11488,706],[89,0,64819,4006],[89,3,93988,4967],[90,0,139384,6427],[90,3,202106,7969],
  [99,0,267608,11775],[99,3,388031,14600],[100,0,287723,12594],[100,3,417199,15616],[124,0,496919,22749],[124,3,720533,28208],
  [125,0,508363,23316],[125,3,737126,28912],[200,0,2803945,147956],[200,3,4065721,183465],[300,0,27325242,1738215],[300,3,39621601,2155387],
  [419,0,410420900,32611402],[419,3,595110304,40438138],[420,0,419872482,33424828],[420,3,608815100,41446787],
  [500,0,2595093477,239908856],[500,3,3762885542,297486981],
];
...
if (process.argv.includes('--self')) {
  ...
  const cases = [
    ['기저 HP·DMG 를 재적합하듯 두 엔진 다 바꾸면 (PLAN 까지 같이 고쳐도 여기선 걸린다)',
      s => s.replace('eBaseHp:49.283586, eBaseDmg:9.164809', 'eBaseHp:44.000000, eBaseDmg:8.100000'),
      s => s.replace('eBaseHp:49.283586, eBaseDmg:9.164809', 'eBaseHp:44.000000, eBaseDmg:8.100000'), null],
    ['10챕터 벽 배수를 올리면',
    ...
    ['구간 성장률 한 칸만 «미세 조정» 하면 (30→60 HP 7.8482% → 7.0%)',
      s => s.replace('[30,1.078482]', '[30,1.070000]'),
      s => s.replace('[30,1.078482]', '[30,1.070000]'), null],
    ...
    ['index.html 만 흔들면 (두 엔진이 갈라진다)',
      null, s => s.replace('eBaseHp:49.283586', 'eBaseHp:52.000000'), null],
