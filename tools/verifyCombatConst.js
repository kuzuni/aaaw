...
const CHECKS=[
  ...
  ['실드 최대치 배수',      /maxSh:Math\.round\(maxHp\*([\d.]+)\)/,     /`maxSh = maxHp\*([\d.]+)`/,                 '§2.3'],
  ...
  ['웨이브 최소 수',        /const waveCount=(\d+)\+\(rnd/,             /웨이브 수: (\d+)~\d+개/,                    '§2.4'],
  ['웨이브 적 수(소)',      /const size=rnd\(\)<[\d.]+\?(\d+):\d+/,     /웨이브당 적 (\d+)마리 또는 \d+마리/,        '§2.4'],
  ['웨이브 적 수(대)',      /const size=rnd\(\)<[\d.]+\?\d+:(\d+)/,     /웨이브당 적 \d+마리 또는 (\d+)마리/,        '§2.4'],
  ['이벤트 가중치 rest',    /r<([\d.]+)\?'rest'/,                       /가중치\s*\((\d+)\/\d+\/\d+%\)/,             '§2.4', 100],
  ['이벤트 가중치 devil',   /r<[\d.]+\?'rest':\(r<([\d.]+)\?'devil'/,   /가중치\s*\((\d+)\/(\d+)\/\d+%\)/,           '§2.4', 100, 'cum2'],
];
