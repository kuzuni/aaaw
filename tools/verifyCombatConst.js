...
const CHECKS=[
  ...
  /* ⚑ T121 2차 — «처치 시 대시»(주인 17:0X)가 이동 한 줄에 `*(p.dash?DASH_MUL:1)` 을 끼웠다.
     기본 속도 상수는 그대로이므로 대시 배수 부분만 선택적으로 읽고 넘어간다. */
  ['플레이어 이동속도',     /p\.worldX\+=(\d+)\*p\.walkMul\*(?:\(p\.dash\?DASH_MUL:1\)\*)?dt/, /속도 `(\d+)\*walkMul`/, '§2.3'],
  ...
  /* ⚑⚑⚑ T163 (주인 확정 2026-09-05 22:1X «지금의 절반으로») — 88 → 44 이고, 배치가 리터럴이 아니라
     `ENEMY_GAP` 상수를 쓴다. 엔진 쪽 값은 그 선언에서 읽는다(리터럴로 되돌아가면 아래 정규식이 못 찾아 빨개진다). */
  ['적 배치 간격',          /const ENEMY_GAP=(\d+);/,                   /적 간격 \*\*(\d+)px\*\*/,                   '§2.3'],
  ...
  /* ⚑ T121 2차 — «반격 강화»(주인 17:0X)가 반격 데미지를 뒤에서 곱하려고 `const` → `let` 이 됐다.
     기본 계수 0.7 자체는 그대로다(곱연산은 그 아래 줄에서 특전이 걸릴 때만 붙는다). */
  ['반격 데미지 계수',      /(?:const|let) cd=effDmg\(p\)\*([\d.]+)\*\(1\+px\.counterX\)/, /반격 데미지 `effDmg\*([\d.]+)\*/, '§4'],
...
console.log('\n=== ④ 스턴 · 빗맞음 축 (PLAN §3.0·§4 주인 지시 15:5X · T48) ===');
{
  ...
  if(at<0) fail('sim.js 에서 CLI 디스패처를 못 찾았다 — 잘림 기준이 바뀌었다');
    ...
    /* ⚑ T121 ⑥ (주인 지시 16:4X) — 유저 노출 용어가 «기절» → «스턴» 으로 통일됐다. PLAN 문면도 같이 바뀌었다. */
    planHas(/스턴 지속은 3초 또는 6초만/)
      ? pass('PLAN §3.0 에 주인 확정 «스턴 지속 3초/6초» 조항이 있다')
      : fail('PLAN §3.0 에서 스턴 조항이 사라졌다');
  }
}
...
console.log('\n=== ⑥ 횟수형 방어막 (PLAN §3.0 주인 17:2X · T48) ===');
{
  ...
  if(at<0) fail('sim.js 에서 CLI 디스패처를 못 찾았다 — 잘림 기준이 바뀌었다');
    ...
    /* (4) ⚑⚑⚑ T121 3차 (주인 확정 2026-09-04 18:2X) — 종전 위임 판단(«막은 타격도 피격이다»)을
       주인이 뒤집었다: «방어막으로 막은 공격은 «피격» 이 아니다(트리거·가시갑옷 발동 없음)».
       판정 순서도 **회피 → 방어막 → 피해 무시 → 피해** 로 못 박혔다. 그래서 이제 단언이 반대다 —
       방어막으로 막으면 피격 트리거가 **하나도** 굴지 않아야 한다(같은 문면인 «피해 무시» 도 마찬가지). */
    {
      ...
      W.hitPlayer(G,300,true,e);
      Math.random=rnd;
      p.buffs.def.length===0&&p.ward===0
        ? pass('막은 타격은 «피격» 이 아니다 — 방어막 1장만 소모하고 피격 트리거는 굴지 않는다 (⚑ 주인 18:2X)')
        : fail(`막은 타격이 아직 «피격» 으로 처리된다 (def버프 ${p.buffs.def.length}건 · 남은 방어막 ${p.ward}장)`);
    }
    /* (4-b) ⚑ T121 3차 «피해 무시»(일반 20%) — 회피·방어막 «뒤» 에 굴리고, 걸리면 역시 «피격» 이 아니다.
       확률 굴림을 전부 성공시켜 무시가 일어나게 하고(난수 0), 피격 트리거·체력 변화가 없는지 본다. */
    {
      const {G,p,e}=mkG({defHitBuff:1});
      p.px.p_ignoreN=1;
      ...
      W.hitPlayer(G,300,true,e);
      Math.random=rnd;
      (p.hp===hp0&&p.sh===sh0&&p.buffs.def.length===0)
        ? pass('«피해 무시» 가 걸린 타격은 데미지도 «피격» 도 없다 (⚑ 주인 18:2X)')
        : fail(`«피해 무시» 가 안 걸리거나 피격 트리거가 굴었다 (hp ${hp0}→${p.hp} · sh ${sh0}→${p.sh} · def버프 ${p.buffs.def.length}건)`);
    }
