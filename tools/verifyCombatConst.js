...
const CHECKS=[
  ...
  /* ⚑ T105 — 매판 굴리던 `const ranged=Math.random()<0.4&&j>0` 이 챕터 시드로 옮겨가며 상수 `RANGED_P` 가 됐다.
     비율 자체는 40% 그대로라 PLAN §2.3 문면과의 대조는 그대로 유효하다. */
  ['원거리 적 비율',        /const RANGED_P=([\d.]+);/,                    /원거리 적\((\d+)% 확률/,                 '§2.3', 100],
  ...
  /* ⚑⚑ T96 4단계 — 챕터 구성이 «전 챕터 고정» 이 되면서 제비뽑기 3항목이 상수 2개로 바뀌었다.
     전수 구성·«완주 = 특전 10개» 는 전용 게이트 `tools/verifyChapterFixed.js` 가 본다. */
  ['웨이브 수(고정)',       /const LAYOUT_WAVES=(\d+),/,                /웨이브 (\d+)개 · 웨이브당/,                '§2.4'],
  ['웨이브당 적 수(고정)',  /LAYOUT_WAVE_SIZE=(\d+),/,                  /웨이브당 (\d+)마리/,                       '§2.4'],
