...
const OWNER_GEAR_TX = ['치명타 확률 +5', '치명타 피해 +20', '반격률 +10', '치명타 피해 +25', '회피 +8',
  ...
  /* ⚑⚑⚑ T145 주인 확정 (2026-09-05 16:4X) — 7번(신화 +6강) 칸 «흡혈 +8%». 주인 원문이 «8퍼» 라
     5% 단위(Ⓕ)로 깎으면 주인이 말한 값이 아니게 된다 — 문면 그대로 면제한다. **워커 수정 금지.** */
  '흡혈 +8%'];
...
console.log('\n[Ⓐ «%p» 표기 금지 — 주인 지시 «게이트의 금지어 검사에 %p 추가»]');
{
  ...
  /* 방어력·회피·반격은 % 스탯이므로 «+8%» 가 맞다(엔진: def/evade/counter 는 가산 후 상한 90/90 의 % 값).
  ...
  /* ⚑⚑⚑ T121 (주인 확정 2026-09-04 16:0X·16:2X·16:3X) — 기존 4종의 값이 내려가고(+10 → +8 · +50 → +30)
     같은 «단위 없는 스탯» 어법의 신규가 늘었다(수집가·치명 +2 · 치명 스택 +1 · 희귀 II 4종 +16).
...
const KNOWN_PROB = [   /* ⚑ T124 — 종전 '장비 plate옵4' 는 18계열 폐지로 대상 소멸 */
  ...
  /* ⚑⚑⚑ T155 (주인 확정 2026-09-05 18:5X «일반에 회피 시 33퍼 … 희귀에 66퍼») — «회피 시 회복» 이
     33/66 어법으로 교체됐다. 주인 원문값이라 **워커 수정 금지**(III 는 100% 라 확률 대상 아님). */
  '특전 p_evHealR'];
...
/* Ⓕ 수치 계수 5% 단위 — 잔여 26건 */
const KNOWN_COEF = [
  /* ⚑⚑⚑ 주인 확정 예외 (T104, 2026-09-03 · ⚑ T155 로 «33% 확률로 최대 체력 12% 회복» 으로 교체).
     주인 원문값이라 워커 수정 금지. 12% 는 셋(I·II·III)이 공유하는 회복량이라 II·III 도 같이 등재한다. */
  '특전 p_evadeHeal', '특전 p_evHealR', '특전 p_evHealL',
  /* ⚑⚑⚑ T121 주인 확정 예외 (2026-09-04 16:0X·16:2X) — 주인이 표에 직접 적은 계수라 워커 수정 금지.
     수집가 «하나당 +4% / +7%» · 처치 시 «+1% / 체력 6% / 실드 6%» · 공격 시 공속 «+7%».
