...
/* 회귀 측정 규약 게이트 (T87 신설 — T74 가 «규약화» 로 남긴 몫)
   ...
   검사:
     ① 결정성 — 같은 SEED 로 두 번 돌린 탐침(실험3 진행 곡선) 출력이 **바이트 단위로 같다**.
...
/* ⚑ T97 — 탐침 판수 40 → 200. 재적합으로 세 시드가 «같은 눈금에 겹쳐 찍히는 일»을 막았다.
   ...
   ⚑⚑⚑ T121 3차 — 그 경고가 세 번째로 현실이 됐다. 주인 확정(T121 3차 특전 하향·T123 기본 스탯 0)으로
   **실험5 여덟 칸이 전부 0.0% 로 포화**해 «일반 풀셋(슬롯0)/챕터 15» 탐침이 세 시드 모두 같은 출력을 냈다.
   칸을 옮겨 봐야 사다리 위 어느 칸도 0 을 벗어나지 못하므로(그것 자체는 승인 대기 51·52번 자리다),
   탐침을 **클리어율이 아니라 «진행 곡선»**(실험3 · 챕터 1~6 · 재시도 상한 200)으로 옮긴다.
   이 출력은 챕터별 시도 횟수라 **클리어율이 0 이든 100 이든 시드마다 갈린다** — 구조적으로 포화하지 않는다.
   자(尺)의 자라서 여기서 재는 것은 여전히 «SEED 가 난수에 물리는가» 뿐이고 밸런스 수치는 한 글자도 안 본다. */
function runSeed(seed) {
  const env = Object.assign({}, process.env, { SEED: String(seed), EXP3_MAX: '6', EXP3_LIMIT: '200' });
  return execFileSync(process.execPath, ['sim.js', '3'], { cwd: root, env, encoding: 'utf8', maxBuffer: 1 << 26 });
}
